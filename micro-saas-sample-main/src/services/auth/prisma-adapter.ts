...
export const supabaseAdapter = {
  ...
  async deleteUser(userId: string) {
    // Supabase não tem delete cascade via cliente, então precisamos deletar as relações primeiro
    // Para simplificar, apenas marcamos como inativo ou removemos as relações
  ...
  async getUserByAccount(account: any) {
    const dbAccount = await db.findAccountByProviderAndAccountId(
      account.provider,
      account.providerAccountId
    )
    if (!dbAccount) return null
    return await db.findUserById(dbAccount.userId)
  },
  ...
  async useVerificationToken(identifier_token: { identifier: string; token: string }) {
    const verificationToken = await db.findVerificationToken(
      identifier_token.identifier,
      identifier_token.token
    )
    if (!verificationToken) return null
    await db.deleteVerificationToken(identifier_token.identifier, identifier_token.token)
