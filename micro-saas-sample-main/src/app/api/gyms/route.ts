...
export async function GET(request: NextRequest) {
  try {
    ...
    const userGyms = await db.findUserGymsByUserId(session.user.id)

    // Filtrar apenas os ativos
    const activeUserGyms = userGyms.filter(ug => ug.status === 'ACTIVE')

    // Se tiver gymId específico e pertencer ao usuário
    if (gymId) {
      const userGym = activeUserGyms.find(ug => ug.gymId === gymId)
      if (!userGym) {
        return NextResponse.json({ error: 'Gym not found or access denied' }, { status: 404 })
      }
      
      // Buscar contagens da academia
    ...
    // Retorna apenas as academias do usuário com contagens
    const gymsWithCounts = await Promise.all(
      activeUserGyms.map(async (ug) => {
        const counts = await db.getGymCounts(ug.gymId)
        return {
          ...ug.gym,
          _count: counts,
          role: ug.role,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (cnpj) {
      ...
      const existingCnpj = allGyms.find(g => g.cnpj === cnpj)
      if (existingCnpj) {
        return NextResponse.json(
    ...
    if (email) {
      const allGyms = await db.findAllGyms()
      const existingEmail = allGyms.find(g => g.email === email)
      if (existingEmail) {
        return NextResponse.json(
