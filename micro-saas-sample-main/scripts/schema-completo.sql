-- ============================================
-- Gym Manager - Schema COMPLETO com Dados de Exemplo
...
-- Aguardar os IDs serem gerados e criar vínculo
-- Nota: Este script será executado em sequência, então precisamos usar subqueries
...
SELECT 
  ...
  u.id, g.id, tr.id
FROM (VALUES
  ('Treino A - Peito e Tríceps', 'Musculação', '60', 'Intermediário', 'Foco em peito e tríceps com exercícios compostos'),
  ('Treino B - Costas e Bíceps', 'Musculação', '60', 'Intermediário', 'Foco em costas e bíceps'),
  ('Aula de Yoga - Iniciantes', 'Yoga', '90', 'Iniciante', 'Aula introdutória de yoga com posturas básicas'),
  ('WOD Cross Training', 'Cross Training', '45', 'Avançado', 'Workout of the Day - Circuito intenso')
...
SELECT 
  ...
  u.id, g.id
FROM (VALUES
  ('Aluguel do Espaço', 'Aluguel mensal da academia', 5000.00, 'Aluguel'),
  ('Conta de Luz', 'Energia elétrica', 800.00, 'Utilities'),
  ('Manutenção de Equipamentos', 'Revisão mensal das máquinas', 1200.00, 'Manutenção'),
  ('Salário Instrutores', 'Pagamento equipe de instrutores', 8000.00, 'Pessoal')
...
SELECT 
  g.id, p.name, p.description, p.price, p.duration, p.maxMembers, p.isActive
FROM (VALUES
